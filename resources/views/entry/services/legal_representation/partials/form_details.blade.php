{{-- resources/views/entry/services/legal_representation/partials/form_details.blade.php --}}
@php
    $item = $legal_representation;
@endphp
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 pb-6 border-b border-gray-50 font-medium font-medium">
    <div class="md:col-span-1">
        <label class="block text-sm font-bold text-gray-700 mb-2 font-bold font-bold">اسم المستفيد <span class="text-rose-500">*</span></label>
    ...
    <div>
        <label class="block text-sm font-bold text-gray-700 mb-2 font-bold font-bold">رقم الهوية <span class="text-rose-500">*</span></label>
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 pt-6 font-medium">
    ...
    <div>
        <label class="block text-sm font-bold text-gray-700 mb-2 font-bold">نوع الاستخراج</label>
        <select name="extract_type"
                class="w-full">
            <option value="">اختر النوع</option>
            <option value="شرعي" {{ old('extract_type', $item->extract_type) == 'شرعي' ? 'selected' : '' }}>شرعي</option>
            <option value="نظامي" {{ old('extract_type', $item->extract_type) == 'نظامي' ? 'selected' : '' }}>نظامي</option>
        </select>
    </div>
    <div class="md:col-span-1">
        <label class="block text-sm font-bold text-gray-700 mb-2 font-bold">تفاصيل الورقة</label>
        <input type="text" name="extract_type_detail" value="{{ old('extract_type_detail', $item->extract_type_detail) }}"
               class="w-full">
    </div>
    <div>
        <label class="block text-sm font-bold text-gray-700 mb-2 font-bold">تاريخ الاستخراج</label>
