...
@section('title', 'خدمات التمثيل القانوني')
@section('page-title', 'سجل التمثيل القانوني (استخراج أوراق)')

@section('content')
<div class="space-y-6 text-right font-medium" dir="rtl">
    <div class="flex items-center justify-between font-bold">
        <p class="text-sm text-gray-500 font-medium font-medium">تعبئة البيانات التفصيلية لاستخراج الأوراق الرسمية</p>
    </div>
                ...
                <tbody class="divide-y divide-gray-50 font-medium">
                    @forelse($cases as $item)
                        <tr class="hover:bg-gray-50/50 transition-colors group">
                            ...
                            <td class="px-6 py-4 text-center text-[11px] text-gray-500 font-bold font-bold font-bold font-bold font-bold">
                                {{ $item->start_date?->format('Y/m/d') }} - {{ $item->end_date?->format('Y/m/d') }}
                            </td>
                            <td class="px-6 py-4 text-center">
                                @if(($item->children_count ?? 0) > 0)
                                    <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-bold border text-indigo-600 bg-indigo-50 border-indigo-100 font-bold">
                                        تم إدراج {{ $item->children_count }} استخراج
                                    </span>
                                @else
                            ...
                            <td class="px-6 py-4 text-left">
                                <div class="flex items-center justify-end gap-1">
                                    <a href="{{ route('data_entry.services.legal_representation.edit', $item) }}" class="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-lg text-[11px] font-bold hover:bg-indigo-100 transition-colors shadow-sm font-bold shadow-sm">
                    ...
                    @empty
                        <tr><td colspan="4" class="px-6 py-20 text-center text-gray-400 font-bold font-medium font-bold">لا يوجد سجلات متاحة حالياً</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
