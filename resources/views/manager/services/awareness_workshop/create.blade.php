...
@section('title', 'إنشاء ورشة توعوية')
@section('page-title', 'إنشاء ورشة توعوية جديدة')
...
<div class="max-w-4xl mx-auto text-right" dir="rtl">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        <div class="p-6 border-b border-gray-50 flex items-center justify-between">
            ...
            <a href="{{ route('project_manager.services.awareness_workshop.index') }}" class="text-sm text-gray-500 hover:text-indigo-600 font-bold transition-all flex items-center gap-1">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
                العودة للسجل
            </a>
        </div>
        
        <form action="{{ route('project_manager.services.awareness_workshop.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="project_name" value="{{ old('project_name') }}" required
                           class="w-full">
                    @error('project_name') <p class="text-rose-500 text-[11px] mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="text" name="funder" value="{{ old('funder') }}" required
                           class="w-full">
                    @error('funder') <p class="text-rose-500 text-[11px] mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="start_date" value="{{ old('start_date') }}" required
                           class="w-full">
                    @error('start_date') <p class="text-rose-500 text-[11px] mt-1">{{ $message }}</p> @enderror
                </div>
                ...
                <div>
                    ...
                    <input type="date" name="end_date" value="{{ old('end_date') }}" required
                           class="w-full">
                    @error('end_date') <p class="text-rose-500 text-[11px] mt-1">{{ $message }}</p> @enderror
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">عدد المستفيدين الإجمالي <span class="text-rose-500">*</span></label>
                    <input type="number" name="total_beneficiaries" value="{{ old('total_beneficiaries', 0) }}" required
                           class="w-full">
                    @error('total_beneficiaries') <p class="text-rose-500 text-[11px] mt-1">{{ $message }}</p> @enderror
                </div>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-gray-50">
                <button type="submit" 
                        class="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-xl text-sm font-bold transition-all shadow-lg shadow-indigo-600/20">
                    حفظ ونشر النشاط
                </button>
            </div>
        </form>
