...
@section('title', 'تفاصيل الاستشارات النفسية')
@section('page-title', $psychological_consultation->project_name)
...
<div class="space-y-6 pb-12 text-right font-medium" dir="rtl">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-800">{{ $psychological_consultation->project_name }}</h2>
                <div class="flex items-center gap-2 mt-1">
                    <span class="px-3 py-1 rounded-full bg-indigo-50 text-indigo-600 text-[10px] font-bold border border-indigo-100 uppercase">استشارات نفسية</span>
                    <span class="text-[11px] text-gray-400 font-bold">بواسطة: {{ $psychological_consultation->submitter?->name }}</span>
                </div>
            </div>
        </div>
    ...
    {{-- ─── STATS GRID ──────────────────────────────────────────────── --}}
    @php
        $childrenAll = $psychological_consultation->children;
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <div class="relative flex flex-col">
                ...
                <span class="text-[11px] text-indigo-500 font-bold mt-1 italic">استشارة نفسية</span>
            </div>
        </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="space-y-1">
                ...
                <p class="text-base font-bold text-slate-700">{{ $psychological_consultation->funding_agency ?: '—' }}</p>
            </div>
            <div class="space-y-1">
                ...
                <p class="text-base font-bold text-slate-700">
                    {{ $psychological_consultation->start_date?->format('Y/m/d') }} — {{ $psychological_consultation->end_date?->format('Y/m/d') }}
                </p>
            </div>
            <div class="space-y-1">
                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">الحالة</p>
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold {{ $psychological_consultation->status == 'active' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-gray-50 text-gray-500 border border-gray-200' }}">
                    {{ $psychological_consultation->status == 'active' ? 'نشط' : 'مكتمل' }}
                </span>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-6">
        ...
        <div class="grid grid-cols-1 gap-6">
                ...
                <div x-show="expanded" x-collapse>
                    <div class="px-8 pb-8 pt-2 border-t border-gray-50 bg-slate-50/20">
                        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6 mt-6">
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">كود الحالة</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->case_code ?: '—' }}</p>
                            </div>
                            <div class="space-y-1">
                            ...
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">المهمة</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->mission ?: '—' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">حالة النزوح</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->displacement_status ?: '—' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">المحافظة الأصلية</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->original_governorate ?: '—' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">العنوان الدائم</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->primary_address ?: '—' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">عنوان النزوح</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->displacement_address ?: '—' }}</p>
                            </div>
                            <div class="space-y-1">
                            ...
                            <div class="space-y-1">
                                ...
                                <p class="text-sm font-black text-slate-700">{{ $child->disability_type ?: 'لا يوجد' }}</p>
                            </div>
                            <div class="space-y-1">
                        ...
                        <div class="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div class="p-6 bg-white rounded-3xl border border-slate-100 shadow-sm">
                                <h5 class="text-[10px] text-indigo-400 font-bold uppercase mb-2">وصف الحالة النفسية</h5>
                                <p class="text-sm text-slate-600 leading-relaxed">{{ $child->case_description }}</p>
                            </div>
                            <div class="p-6 bg-emerald-50/30 rounded-3xl border border-emerald-100 shadow-sm">
                                <h5 class="text-[10px] text-emerald-500 font-bold uppercase mb-2">الإجراء / التوجيه المتبع</h5>
                                <p class="text-sm text-slate-600 leading-relaxed">{{ $child->procedure_guidance }}</p>
                            </div>
                            <div class="p-6 bg-amber-50/30 rounded-3xl border border-amber-100 shadow-sm">
                                <h5 class="text-[10px] text-amber-500 font-bold uppercase mb-2">التوصيات</h5>
                                <p class="text-sm text-slate-600 leading-relaxed">{{ $child->recommendations ?: '—' }}</p>
                            </div>
                        </div>

                        @if($child->description || $child->project_description)
                        <div class="mt-6 grid grid-cols-1 md:grid-cols-2 gap-6">
                            @if($child->description)
                            <div class="p-6 bg-slate-50 rounded-3xl border border-slate-100 shadow-sm">
                                <h5 class="text-[10px] text-slate-500 font-bold uppercase mb-2">وصف النشاط</h5>
                                <p class="text-sm text-slate-600 leading-relaxed">{{ $child->description }}</p>
                            </div>
                            @endif
                            @if($child->project_description)
                            <div class="p-6 bg-slate-50 rounded-3xl border border-slate-100 shadow-sm">
                                <h5 class="text-[10px] text-slate-500 font-bold uppercase mb-2">وصف المشروع</h5>
                        ...
                        <div class="mt-8">
                            <h5 class="text-[11px] font-black text-slate-800 mb-4">المرفقات والوثائق الإرشادية</h5>
            ...
            <div class="bg-white rounded-3xl p-20 text-center border-2 border-dashed border-gray-100">
                <p class="text-sm font-bold text-gray-400">لا توجد استشارات نفسية معتمدة حالياً.</p>
            </div>
            @endforelse
        </div>
    </div>
