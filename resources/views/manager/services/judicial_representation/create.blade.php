...
@section('title', 'إنشاء سجل تمثيل قضائي')
@section('page-title', 'إدارة سجلات التمثيل القضائي')
...
<div class="max-w-4xl mx-auto text-right font-medium" dir="rtl">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden font-medium">
        <div class="p-6 border-b border-gray-50 flex items-center justify-between font-bold bg-gray-50/50">
            ...
            <a href="{{ route('project_manager.services.judicial_representation.index') }}" class="text-sm text-gray-500 hover:text-indigo-600 font-bold transition-all flex items-center gap-1 font-bold">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
                العودة
            </a>
        </div>
        
        <form action="{{ route('project_manager.services.judicial_representation.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 font-medium font-bold">
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2 font-bold">اسم المشروع / النشاط <span class="text-rose-500">*</span></label>
