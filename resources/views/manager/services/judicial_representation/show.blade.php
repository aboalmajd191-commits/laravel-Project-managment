...
@section('title', 'تفاصيل التمثيل القضائي')
@section('page-title', $judicial_representation->project_name)
...
<div class="space-y-6 pb-12 text-right font-medium" dir="rtl">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-800">{{ $judicial_representation->project_name }}</h2>
                <div class="flex items-center gap-2 mt-1">
                    <span class="px-3 py-1 rounded-full bg-indigo-50 text-indigo-600 text-[10px] font-bold border border-indigo-100 uppercase">التمثيل القضائي</span>
                    <span class="text-[11px] text-gray-400 font-bold">بواسطة: {{ $judicial_representation->submitter?->name }}</span>
                </div>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('project_manager.services.judicial_representation.edit', $judicial_representation) }}" 
            ...
            <a href="{{ route('project_manager.services.judicial_representation.edit', [$judicial_representation, 'tab' => 'entry']) }}"
    ...
    {{-- ─── STATS GRID ──────────────────────────────────────────────── --}}
    @php
        $childrenAll = $judicial_representation->children;
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <div class="relative flex flex-col">
                ...
                <span class="text-[11px] text-indigo-500 font-bold mt-1 italic">تمثيل قضائي</span>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <div class="relative flex flex-col">
                <span class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">إجمالي المبالغ المحصلة</span>
                <span class="text-3xl font-black text-emerald-600">{{ number_format($approvedChildren->sum('total_paid')) }}</span>
                <span class="text-[11px] text-emerald-500 font-bold mt-1 tracking-widest uppercase italic">عملة المشروع</span>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <div class="relative flex flex-col">
                <span class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">حالات قيد النظر</span>
                <span class="text-3xl font-black text-purple-600">{{ $approvedChildren->where('case_status','pending')->count() }}</span>
                <span class="text-[11px] text-purple-400 font-bold mt-1 italic tracking-widest">قضية مفتوحة</span>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <div class="relative flex flex-col">
                ...
                <span class="text-[11px] text-amber-400 font-bold mt-1 tracking-widest italic">تحتاج مراجعة</span>
            </div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="space-y-1">
                ...
                <p class="text-base font-bold text-slate-700">{{ $judicial_representation->funding_agency ?: '—' }}</p>
            </div>
            <div class="space-y-1">
                ...
                <p class="text-base font-bold text-slate-700">
                    {{ $judicial_representation->start_date?->format('Y/m/d') }} — {{ $judicial_representation->end_date?->format('Y/m/d') }}
                </p>
            </div>
            <div class="space-y-1">
                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">الحالة</p>
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold {{ $judicial_representation->status == 'active' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-gray-50 text-gray-500 border border-gray-200' }}">
                    {{ $judicial_representation->status == 'active' ? 'نشط' : 'مكتمل' }}
                </span>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between pr-4">
            <h3 class="text-lg font-bold text-slate-800 border-r-4 border-emerald-500 pr-4">سجل التمثيل القضائي المعتمد</h3>
            <span class="text-xs font-bold text-gray-400">{{ $approvedChildren->count() }} تمثيل مقبول</span>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6">
                ...
                <div class="p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div class="flex items-center gap-5 flex-1">
                        ...
                        <div class="min-w-0">
                            <h4 class="text-lg font-bold text-slate-800 truncate">
                                ...
                                <span class="text-xs text-gray-400 font-normal mr-2">/ رقم القضية: {{ $child->case_number ?: '—' }}</span>
                            </h4>
                            <div class="flex flex-wrap items-center gap-x-6 gap-y-2 mt-2 text-xs text-gray-400 font-bold">
                                <span>⚖️ {{ $child->case_type ?: '—' }}</span>
                                <span>🏛️ {{ $child->court_name ?: '—' }}</span>
                    ...
                    <div class="flex items-center gap-4">
                        <div class="text-left bg-emerald-50 px-5 py-2.5 rounded-2xl border border-emerald-100">
                            <p class="text-[9px] text-emerald-400 font-bold uppercase tracking-widest mb-0.5">المبلغ المحصل</p>
                            <p class="text-lg font-black text-emerald-600">{{ number_format($child->total_paid) }}</p>
                        </div>
                ...
                <div x-show="expanded" x-collapse>
                    <div class="px-8 pb-8 pt-2 border-t border-gray-50 bg-slate-50/20">
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mt-6">
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">الاسم (EN)</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->name_en ?: '—' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">المنطقة</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->region ?: '—' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">رقم الجوال</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->mobile ?: '—' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">نوع الإعاقة</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->disability_type ?: '—' }}</p>
                            </div>
                            <div class="space-y-1">
                            ...
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">الحالة الصحية</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->health_status ?: '—' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">عدد القضايا</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->cases_count ?: '—' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">اسم / نوع الدعوى</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->case_name_type ?: '—' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">تاريخ رفع الدعوى</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->lawsuit_date ? $child->lawsuit_date->format('Y/m/d') : '—' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">تاريخ الإغلاق</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->closing_date ? $child->closing_date->format('Y/m/d') : '—' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">حالة القضية</p>
                                <span class="inline-flex px-3 py-1 rounded-full text-[10px] font-bold bg-indigo-50 text-indigo-600 border border-indigo-100">{{ $child->case_status ?: '—' }}</span>
                            </div>
                            <div class="space-y-1">
                            ...
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">تاريخ الإدخال</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
                            ...
                            <div class="p-6 bg-white rounded-3xl border border-slate-100 shadow-sm">
                                <h5 class="text-[10px] text-slate-400 font-bold uppercase mb-2">وصف النشاط</h5>
                                <p class="text-sm text-slate-600 leading-relaxed">{{ $child->description }}</p>
                            </div>
                            @endif

                            @if($child->project_description)
                            <div class="p-6 bg-white rounded-3xl border border-slate-100 shadow-sm">
                                <h5 class="text-[10px] text-slate-400 font-bold uppercase mb-2">وصف المشروع</h5>
                                <p class="text-sm text-slate-600 leading-relaxed">{{ $child->project_description }}</p>
                            </div>
                            @endif
                        </div>
                        ...
                        <div class="mt-8">
                            <h5 class="text-[11px] font-black text-slate-800 mb-4">المرفقات والوثائق القضائية</h5>
            ...
            <div class="bg-white rounded-3xl p-20 text-center border-2 border-dashed border-gray-100">
                <p class="text-sm font-bold text-gray-400">لا توجد حالات تمثيل قضائي معتمدة حالياً.</p>
            </div>
            @endforelse
        </div>
    </div>
