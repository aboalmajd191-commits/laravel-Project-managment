...
@section('title', 'تعديل الاستشارات القانونية')
@section('page-title', 'تحديث سجل الاستشارات القانونية')
...
<div class="max-w-5xl mx-auto text-right font-medium" dir="rtl">
    ...
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            ...
            <div class="flex items-center gap-1">
                ...
                <a href="{{ route('project_manager.services.legal_consultation.edit', [$legal_consultation, 'tab' => 'entry']) }}"
                   class="px-5 py-4 text-sm font-bold border-b-2 transition-all {{ request('tab') === 'entry' ? 'border-emerald-600 text-emerald-600' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
                    ➕ إضافة استشارة
                    @if($legal_consultation->children_count > 0)
                        <span class="mr-1.5 inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-100 text-emerald-600">
                            {{ $legal_consultation->children_count }}
                        </span>
                    @endif
                </a>
            </div>
            <a href="{{ route('project_manager.services.legal_consultation.index') }}"
        ...
        <form action="{{ route('project_manager.services.legal_consultation.update', $legal_consultation) }}" method="POST" class="p-6">
            @csrf @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="funder" value="{{ old('funder', $legal_consultation->funder) }}" required
                           class="w-full">
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">اسم المشروع <span class="text-rose-500">*</span></label>
                    <input type="text" name="project_name" value="{{ old('project_name', $legal_consultation->project_name) }}" required
                           class="w-full">
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">بدء المشروع <span class="text-rose-500">*</span></label>
                    <input type="date" name="start_date" value="{{ old('start_date', $legal_consultation->start_date?->format('Y-m-d')) }}" required
                           class="w-full">
                </div>

                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2">نهاية المشروع <span class="text-rose-500">*</span></label>
                    <input type="date" name="end_date" value="{{ old('end_date', $legal_consultation->end_date?->format('Y-m-d')) }}" required
                           class="w-full">
                </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <select name="status" required class="w-full">
                            <option value="active"    {{ old('status', $legal_consultation->status) == 'active'    ? 'selected' : '' }}>نشط (متاح للإدخال)</option>
                            <option value="draft"     {{ old('status', $legal_consultation->status) == 'draft'     ? 'selected' : '' }}>مسودة</option>
                            <option value="completed" {{ old('status', $legal_consultation->status) == 'completed' ? 'selected' : '' }}>مكتمل</option>
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="sector_type" required class="w-full">
                            <option value="" disabled {{ !$legal_consultation->sector_type ? 'selected' : '' }}>اختر النوع...</option>
                            <option value="protection" {{ old('sector_type', $legal_consultation->sector_type) == 'protection' ? 'selected' : '' }}>الحماية</option>
                            <option value="education"   {{ old('sector_type', $legal_consultation->sector_type) == 'education' ? 'selected' : '' }}>التعليم</option>
                            <option value="health"      {{ old('sector_type', $legal_consultation->sector_type) == 'health' ? 'selected' : '' }}>الصحة</option>
                        </select>
                    </div>
                </div>
            </div>
                ...
                <div class="mt-6">
                    ...
                    <textarea name="project_description" rows="3"
                              class="w-full">{{ old('project_description', $legal_consultation->project_description) }}</textarea>
                </div>
            </div>
        ...
        {{-- Entries list --}}
        @if($legal_consultation->children->isNotEmpty())
        <div class="border-t border-gray-50 px-6 pb-6">
            <h4 class="text-sm font-bold text-gray-700 mt-6 mb-4">الاستشارات المُدخَلة ({{ $legal_consultation->children->count() }})</h4>
            <div class="space-y-3">
                @foreach($legal_consultation->children as $child)
        ...
        {{-- ─── TAB: Entry Data ─── --}}
        @else
        <form action="{{ route('project_manager.services.legal_consultation.update', $legal_consultation) }}" method="POST" class="p-6 space-y-6">
            @csrf @method('PUT')
            ...
            <div class="bg-emerald-50 border border-emerald-100 rounded-xl p-4 flex items-center gap-3">
                ...
                <div>
                    <p class="text-xs text-emerald-600 font-bold">إضافة استشارة قانونية للمشروع: {{ $legal_consultation->project_name }}</p>
                </div>
            </div>

            @include('entry.services.legal_consultation.partials.form_details', ['legal_consultation' => new \App\Models\LegalConsultation()])
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-gray-50">
                ...
                <a href="{{ route('project_manager.services.legal_consultation.index') }}" class="text-sm text-gray-400 hover:text-gray-600 px-4 font-bold">الانتهاء</a>
            </div>
        </form>
        @endif
    </div>
