...
@section('title', 'تفاصيل الوساطة')
@section('page-title', $mediation->project_name)
...
<div class="space-y-6 pb-12 text-right font-medium" dir="rtl">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-800">{{ $mediation->project_name }}</h2>
                <div class="flex items-center gap-2 mt-1">
                    <span class="px-3 py-1 rounded-full bg-indigo-50 text-indigo-600 text-[10px] font-bold border border-indigo-100 uppercase">الوساطة</span>
                    <span class="text-[11px] text-gray-400 font-bold">بواسطة: {{ $mediation->submitter?->name }}</span>
                </div>
            </div>
        </div>
    ...
    {{-- ─── STATS GRID ──────────────────────────────────────────────── --}}
    @php
        $childrenAll = $mediation->children;
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <div class="relative flex flex-col">
                <span class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">إجمالي الوساطات المعتمدة</span>
                <span class="text-3xl font-black text-slate-800">{{ $approvedChildren->count() }}</span>
                <span class="text-[11px] text-indigo-500 font-bold mt-1 italic">حالة وساطة</span>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <div class="relative flex flex-col">
                <span class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">إجمالي المبالغ المستخلصة</span>
                <span class="text-3xl font-black text-emerald-600">{{ number_format($approvedChildren->sum('amount')) }}</span>
                <span class="text-[11px] text-emerald-500 font-bold mt-1 tracking-widest italic">عملة المشروع</span>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <div class="relative flex flex-col">
                ...
                <span class="text-[11px] text-purple-400 font-bold mt-1 italic tracking-widest">لكل وساطة</span>
            </div>
        </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="space-y-1">
                ...
                <p class="text-base font-bold text-slate-700">{{ $mediation->funding_agency ?: '—' }}</p>
            </div>
            <div class="space-y-1">
                ...
                <p class="text-base font-bold text-slate-700">
                    {{ $mediation->start_date?->format('Y/m/d') }} — {{ $mediation->end_date?->format('Y/m/d') }}
                </p>
            </div>
            <div class="space-y-1">
                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">الحالة</p>
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold {{ $mediation->status == 'active' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-gray-50 text-gray-500 border border-gray-200' }}">
                    {{ $mediation->status == 'active' ? 'نشط' : 'مكتمل' }}
                </span>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between pr-4">
            <h3 class="text-lg font-bold text-slate-800 border-r-4 border-emerald-500 pr-4">سجل الوساطات المعتمدة</h3>
            <span class="text-xs font-bold text-gray-400">{{ $approvedChildren->count() }} وساطة مقبولة</span>
        </div>
        ...
        <div class="grid grid-cols-1 gap-6">
                    ...
                    <div class="flex items-center gap-5 flex-1">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="flex flex-wrap items-center gap-x-6 gap-y-2 mt-2 text-xs text-gray-400 font-bold">
                                <span>🏷️ {{ $child->mediation_type ?: '—' }}</span>
                                <span>📅 {{ $child->extraction_date ?: '—' }}</span>
            ...
            <div class="bg-white rounded-3xl p-20 text-center border-2 border-dashed border-gray-100">
                <p class="text-sm font-bold text-gray-400">لا توجد وساطات معتمدة حالياً.</p>
            </div>
            @endforelse
        </div>
    </div>
