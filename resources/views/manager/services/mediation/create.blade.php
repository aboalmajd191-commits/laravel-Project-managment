...
@section('title', 'إنشاء سجل وساطة')
@section('page-title', 'إدارة سجلات الوساطة')
...
<div class="max-w-4xl mx-auto text-right font-medium font-bold" dir="rtl">
    <div class="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden font-medium">
        <div class="p-6 border-b border-gray-50 flex items-center justify-between font-bold bg-gray-50/50">
            <h3 class="text-lg font-bold text-gray-800">بيانات سجل الوساطة (مدير المشروع)</h3>
            <a href="{{ route('project_manager.services.mediation.index') }}" class="text-sm text-gray-500 font-bold hover:text-indigo-600 transition-all flex items-center gap-1 font-bold">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
                العودة
            </a>
        </div>
        
        <form action="{{ route('project_manager.services.mediation.store') }}" method="POST" class="p-6 space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 font-medium">
                ...
                <div>
                    <label class="block text-sm font-bold text-gray-700 mb-2 font-bold">جهة التمويل <span class="text-rose-500">*</span></label>
            ...
            <div class="flex items-center gap-3 pt-4 border-t border-gray-50 font-bold">
                <button type="submit" 
                        class="bg-indigo-600 hover:bg-indigo-700 text-white px-8 py-3 rounded-xl text-sm font-bold shadow-lg shadow-indigo-600/20 font-bold shadow-sm">
                    حفظ وإتاحة الإدخال
                </button>
            </div>
        </form>
