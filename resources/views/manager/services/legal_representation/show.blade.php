...
@section('title', 'تفاصيل التمثيل القانوني')
@section('page-title', $legal_representation->project_name)
...
<div class="space-y-6 pb-12 text-right font-medium" dir="rtl">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-800">{{ $legal_representation->project_name }}</h2>
                <div class="flex items-center gap-2 mt-1">
                    <span class="px-3 py-1 rounded-full bg-indigo-50 text-indigo-600 text-[10px] font-bold border border-indigo-100 uppercase">التمثيل القانوني</span>
                    <span class="text-[11px] text-gray-400 font-bold">بواسطة: {{ $legal_representation->submitter?->name }}</span>
                </div>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('project_manager.services.legal_representation.edit', $legal_representation) }}" 
            ...
            <a href="{{ route('project_manager.services.legal_representation.edit', [$legal_representation, 'tab' => 'entry']) }}"
    ...
    {{-- ─── STATS GRID ──────────────────────────────────────────────── --}}
    @php
        $childrenAll = $legal_representation->children;
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <div class="relative flex flex-col">
                ...
                <span class="text-[11px] text-indigo-500 font-bold mt-1 italic">تمثيل قانوني</span>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <div class="relative flex flex-col">
                ...
                <span class="text-3xl font-black text-emerald-600">{{ number_format($approvedChildren->sum('amount')) }}</span>
        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <div class="relative flex flex-col">
                <span class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">متوسط المبلغ / حالة</span>
                <span class="text-3xl font-black text-purple-600">{{ $approvedChildren->count() > 0 ? number_format($approvedChildren->sum('amount') / $approvedChildren->count()) : 0 }}</span>
                <span class="text-[11px] text-purple-400 font-bold mt-1 italic tracking-widest">لكل تمثيل</span>
            </div>
        </div>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
            <div class="space-y-1">
                ...
                <p class="text-base font-bold text-slate-700">{{ $legal_representation->funding_agency ?: '—' }}</p>
            </div>
            <div class="space-y-1">
                ...
                <p class="text-base font-bold text-slate-700">
                    {{ $legal_representation->start_date?->format('Y/m/d') }} — {{ $legal_representation->end_date?->format('Y/m/d') }}
                </p>
            </div>
            <div class="space-y-1">
                <p class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">الحالة</p>
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold {{ $legal_representation->status == 'active' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-gray-50 text-gray-500 border border-gray-200' }}">
                    {{ $legal_representation->status == 'active' ? 'نشط' : 'مكتمل' }}
                </span>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between pr-4">
            <h3 class="text-lg font-bold text-slate-800 border-r-4 border-emerald-500 pr-4">سجل التمثيل القانوني المعتمد</h3>
        ...
        <div class="grid grid-cols-1 gap-6">
                ...
                <div class="p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div class="flex items-center gap-5 flex-1">
                        ...
                        <div class="min-w-0">
                            ...
                            <div class="flex flex-wrap items-center gap-x-6 gap-y-2 mt-2 text-xs text-gray-400 font-bold">
                                <span>📄 {{ $child->extract_type ?: '—' }}</span>
                                <span>🔍 {{ $child->extract_type_detail ?: '—' }}</span>
                    ...
                    <div class="flex items-center gap-4">
                        <div class="text-left bg-emerald-50 px-5 py-2.5 rounded-2xl border border-emerald-100">
                            <p class="text-[9px] text-emerald-400 font-bold uppercase tracking-widest mb-0.5">المبلغ</p>
                            <p class="text-lg font-black text-emerald-600">{{ number_format($child->amount) }}</p>
                        </div>
                ...
                <div x-show="expanded" x-collapse>
                    <div class="px-8 pb-8 pt-2 border-t border-gray-50 bg-slate-50/20">
                        <div class="grid grid-cols-2 md:grid-cols-4 gap-6 mt-6">
                            ...
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">تاريخ الاستخراج</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->extraction_date ? $child->extraction_date->format('Y/m/d') : '—' }}</p>
                            </div>
                            <div class="space-y-1">
            ...
            <div class="bg-white rounded-3xl p-20 text-center border-2 border-dashed border-gray-100">
                <p class="text-sm font-bold text-gray-400">لا توجد حالات تمثيل قانوني معتمدة حالياً.</p>
            </div>
            @endforelse
        </div>
    </div>
