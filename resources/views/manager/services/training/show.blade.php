...
@section('title', 'تفاصيل النشاط التدريبي')
@section('page-title', $training->name)
...
<div class="space-y-6 pb-12 text-right font-medium" dir="rtl">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h2 class="text-xl font-bold text-slate-800">{{ $training->name }}</h2>
                <div class="flex items-center gap-2 mt-1">
                    <span class="px-3 py-1 rounded-full bg-indigo-50 text-indigo-600 text-[10px] font-bold border border-indigo-100 uppercase">تدريب وبناء قدرات</span>
                    <span class="text-[11px] text-gray-400 font-bold">بواسطة: {{ $training->submitter?->name }}</span>
                </div>
            </div>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('project_manager.services.training.edit', $training) }}" 
            ...
            <a href="{{ route('project_manager.services.training.edit', [$training, 'tab' => 'entry']) }}"
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <div class="relative flex flex-col">
                ...
                <span class="text-[11px] text-indigo-500 font-bold mt-1 italic">متدرب مخطط له</span>
            </div>
        </div>
        ...
        <div class="bg-white p-6 rounded-3xl shadow-sm border border-gray-100 relative overflow-hidden group">
            ...
            <div class="relative flex flex-col">
                <span class="text-[10px] text-gray-400 font-bold uppercase tracking-widest mb-1">دورات معتمدة</span>
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
        ...
        <div class="p-8 grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            <div class="space-y-1">
                ...
                <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold {{ $training->status == 'active' ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-gray-50 text-gray-500 border border-gray-200' }}">
                    <span class="w-1.5 h-1.5 rounded-full {{ $training->status == 'active' ? 'bg-emerald-500' : 'bg-gray-400' }}"></span>
                    {{ $training->status == 'active' ? 'نشط حالياً' : 'مكتمل/غير نشط' }}
                </span>
            </div>
        </div>
    </div>
    ...
    <div class="space-y-6">
        <div class="flex items-center justify-between pr-4">
            ...
            <span class="text-xs font-bold text-gray-400">{{ $approvedChildren->count() }} دورة مقبولة</span>
        </div>
            ...
            <div class="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden transition-all hover:shadow-md" x-data="{ expanded: false, showAttendees: false }">
                ...
                <div class="p-8 flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    <div class="flex items-center gap-5 flex-1">
                        ...
                        <div class="min-w-0">
                            <h4 class="text-lg font-bold text-slate-800 truncate">{{ $child->activity_name ?: ($child->name ?: 'إدخال بدون اسم') }}</h4>
                            @if($child->activity_name)
                                <p class="text-[10px] text-gray-400 font-bold mt-1">الخدمة: {{ $child->name }}</p>
                            @endif
                            <div class="flex flex-wrap items-center gap-x-6 gap-y-2 mt-2 text-xs text-gray-400">
                                <span class="flex items-center gap-1.5 font-bold"><svg class="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 11a3 3 0 11-6 0 3 3 0 016 0z"/></svg> {{ $child->governorate ?: '—' }}</span>
                                <span class="flex items-center gap-1.5 font-bold"><svg class="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg> {{ $child->start_date?->format('Y/m/d') }} — {{ $child->end_date?->format('Y/m/d') }}</span>
                ...
                <div x-show="expanded" x-collapse>
                    <div class="px-8 pb-8 pt-2 border-t border-gray-50">
                        <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-6 mt-6">
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">اسم النشاط</p>
                                <p class="text-sm font-black text-indigo-600">{{ $child->activity_name ?: '—' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">عدد المتدربين</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->attendees->count() }} متدرب</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">الجنس المستهدف</p>
                                <p class="text-sm font-black text-slate-700">{{ $child->gender_type == 'both' ? 'كلاهما' : ($child->gender_type == 'male' ? 'ذكور' : 'إناث') }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[9px] text-gray-400 font-bold uppercase tracking-widest mb-1">المنطقة/المحافظة</p>
                        ...
                        <div class="mt-8 p-6 bg-slate-50 rounded-3xl border border-slate-100">
                            <h5 class="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-2">وصف التدريب / ملاحظات التنفيذ</h5>
                ...
                <div x-show="showAttendees" x-collapse class="bg-slate-50 border-t border-gray-100 overflow-hidden">
                    <div class="p-8">
                        <div class="flex items-center justify-between mb-4 pr-2">
                            <h5 class="text-sm font-bold text-slate-700">قائمة المتدربين الحاضرين ({{ $child->attendees->count() }})</h5>
                        ...
                        <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden shadow-sm">
                            <table class="w-full text-[11px] text-right">
                                ...
                                <tbody class="divide-y divide-gray-50">
                                    ...
                                    @empty
                                    <tr><td colspan="6" class="px-6 py-10 text-center text-gray-400 font-bold italic">لا توجد بيانات متدربين مسجلة</td></tr>
                                    @endforelse
                                </tbody>
                            </table>
                        </div>
